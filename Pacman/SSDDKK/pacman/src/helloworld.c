...
// These hard code the movemnet for the ghosts to get out of their starting box
static const int8_t blue_path[][3] = {
    { 1,  0, 16}, // right
    { 0, -1, 24}, // up
    { 0,  0,  0}  // end
};

static const int8_t orange_path[][3] = {
    {-1,  0, 16}, // left
    { 0, -1, 24}, // up
    { 0,  0,  0}
};

static const int8_t pink_path[][3] = {
    { 0, -1, 24}, // just up
    { 0,  0,  0}
};
...
void game_loop() {
    ...
    // Red (Blinky) starts outside the box
    init_ghost(&red, 13, 14, 0, 2, NULL);

    // Pinky starts in center of box
    init_ghost(&pink, 13, 17, 100, 0, pink_path);
    pink.x += 4;

    // Blue starts left of center
    init_ghost(&blue, 11, 17, 200, 0, blue_path);
    blue.x += 4;

    // Orange starts right of center
    init_ghost(&orange, 15, 17, 400, 0, orange_path);
    orange.x += 4;
    ...
    move_pacman:
        ...
        // This does the tunnel check so pacman and ghosts can use the tunnel
        ...
        if(red.x < LEFT_TUNNEL_X_LIMIT){
            red.x = RIGHT_TUNNEL_X_LIMIT - 7;
        } 
        if(red.x > RIGHT_TUNNEL_X_LIMIT){
            red.x = LEFT_TUNNEL_X_LIMIT + 7;
        }

        if(pink.x < LEFT_TUNNEL_X_LIMIT){
            pink.x = RIGHT_TUNNEL_X_LIMIT - 7;
        } 
        if(pink.x > RIGHT_TUNNEL_X_LIMIT){
            pink.x = LEFT_TUNNEL_X_LIMIT + 7;
        }

        if(blue.x < LEFT_TUNNEL_X_LIMIT){
            blue.x = RIGHT_TUNNEL_X_LIMIT - 7;
        } 
        if(blue.x > RIGHT_TUNNEL_X_LIMIT){
            blue.x = LEFT_TUNNEL_X_LIMIT + 7;
        }

        if(orange.x < LEFT_TUNNEL_X_LIMIT){
            orange.x = RIGHT_TUNNEL_X_LIMIT - 7;
        } 
        if(orange.x > RIGHT_TUNNEL_X_LIMIT){
            orange.x = LEFT_TUNNEL_X_LIMIT + 7;
        }

        align_pacman_center(&pacman_x, &pacman_y, current_dir);


        // calculate pacmans tile coordinates
        ...
        // if power pellet eat it then enable reversed movement for ghosts
        if( tile == 0x02){
            write_tile(aligned_x, aligned_y, 0x00);  // Clear pellet
        ...
        // When we eat all the pellets, reset the map and ghosts
        ...
        // update ghosts every 4 ticks
        // Get center of Pac-Man to pass as BFS target
        int pacman_cx = pacman_x + PACMAN_CENTER;
        int pacman_cy = pacman_y + PACMAN_CENTER;

        // Ghost logic runs even if Pac-Man stands still
        if (rand_counter % 4 == 0) {
            update_ghost(&red, pacman_cx, pacman_cy, current_dir);
            update_ghost(&pink, pacman_cx, pacman_cy, current_dir);
            update_ghost(&blue, pacman_cx, pacman_cy, current_dir);
            update_ghost(&orange, pacman_cx, pacman_cy, current_dir);
        }

        // determines if ther is overlap between pacman and any of the ghosts
        ...
        // if there is reset pacman, ghosts amd decrement lives
        if(check_red == 1 || check_blue == 1 || check_pink == 1 || check_orange == 1){
            ...
            orange.x += 4;
            sleep(1);
        } else if(check_red == 2 || check_blue == 2 || check_pink == 2 || check_orange == 2) { //otherwise eat the ghost
            score += 200;
            update_score(score);
            if(score > high_score){
                high_score = score;
        ...
        // update life counter at bottom of map
        if(lives == 2){
        ...
        // reset game logic i.e) ran out of lives
        if(lives < 1){
            lives = 3;
            score = 0;
            pellet_count = 0;
        ...
        // ghost scared timer (essentially)
...
// Pass an X and Y Pixel we return the tile
uint8_t read_tile(uint16_t pixel_x, uint16_t pixel_y) {
...
// pass and X and Y pixel, and a tile number we change that tile to the passed tile
void write_tile(uint16_t pixel_x, uint16_t pixel_y, uint8_t tile){
...
// pass a tile index we write the given tile
void write_number(uint16_t tile_x, uint16_t tile_y, uint8_t tile){
    uint32_t addr = (tile_y * MAZE_WIDTH + tile_x) * 4;
    Xil_Out32(HDMI_BASE_ADDR + addr, tile);
    return;
...
// pass a tile x and y and a number (0-9) and we will write that specific numebr to the tile
void draw_number(uint16_t tile_x, uint16_t tile_y, uint8_t number){
    switch(number){
    case 1:
...
// pass a direction and a pixel coordinate we return 1 if we can move there and a 0 otherwise
int valid_move(int direction, uint16_t pacman_x, uint16_t pacman_y){
...
// function that resets the pellets and power pellets on the map
void reset_map(){
...
// This is the function that checks the collisions for pacman and the ghosts
...
// when a ghost dies we send him back to the starting box
void ghost_die(Ghost* ghost){
    init_ghost(ghost, 13, 17, 0, 0, pink_path);
    ghost->x += 4;
}

// When the game is over we reset the score and reset all the pelletes on the map
void game_over(){
...
// This aligns pacman to the center of the walkable tiles so that he doesnt clip into the walls
